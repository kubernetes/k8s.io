...
resource "ibm_sm_iam_credentials_secret" "prow_secret" {
  ...
  instance_id   = var.secrets_manager_id
  description   = "API key used for running the Prow job on the ppc64le platform."
  region        = local.secrets_manager_region
...
data "ibm_sm_iam_credentials_secret" "janitor_secret" {
  instance_id = var.secrets_manager_id
  region      = local.secrets_manager_region
  secret_id   = ibm_sm_iam_credentials_secret.janitor_secret.secret_id
}

resource "ibm_iam_access_group_policy" "janitor_iam" {
  access_group_id = var.janitor_access_group_id
  roles           = ["JanitorIAMRole"]

  resources {
    service  = "iam-identity"
    resource = data.ibm_sm_iam_credentials_secret.janitor_secret.service_id
  }
}
