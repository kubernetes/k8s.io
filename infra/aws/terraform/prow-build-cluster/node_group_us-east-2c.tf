...
locals {
  node_group_build_us_east_2c = {
    name            = "build-us-east-2c"
    description     = "EKS managed node group in US East 2c"
    use_name_prefix = true

    cluster_version = var.node_group_version_us_east_2c

    taints = var.node_taints_build
    labels = var.node_labels_build

    # Subnet is US East 2c
    subnet_ids = [module.vpc.public_subnets[2]]

    min_size     = var.node_min_size_us_east_2c
    max_size     = var.node_max_size_us_east_2c
    desired_size = var.node_desired_size_us_east_2c
    ...
    instance_types = var.node_instance_types_us_east_2c
