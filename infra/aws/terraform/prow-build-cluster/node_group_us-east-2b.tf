...
locals {
  node_group_build_us_east_2b = {
    name            = "build-us-east-2b"
    description     = "EKS managed node group in US East 2b"
    use_name_prefix = true

    cluster_version = var.node_group_version_us_east_2b

    taints = var.node_taints_build
    labels = var.node_labels_build

    # Subnet is US East 2b
    subnet_ids = [module.vpc.public_subnets[1]]

    min_size     = var.node_min_size_us_east_2b
    max_size     = var.node_max_size_us_east_2b
    desired_size = var.node_desired_size_us_east_2b
    ...
    instance_types = var.node_instance_types_us_east_2b
