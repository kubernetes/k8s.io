...
# Grant CNS browse access for the janitor.
## /

resource "vsphere_entity_permissions" "permissions_global" {
  entity_id   = data.vsphere_folder.global.id
  entity_type = "Folder"
  permissions {
    user_or_group = var.vsphere_janitor_iam_group
    propagate     = false
    is_group      = true
    role_id       = vsphere_role.vsphere-ci-janitor.id
  }
}