...
locals {
  ...
  cluster_api_provider_vsphere_group = "vsphere.local\\prow-${local.cluster_api_provider_vsphere_project_name}-users"
}
...
module "cluster-api-provider-vsphere" {
  ...
  group = local.cluster_api_provider_vsphere_group
  nr_projects = 5
